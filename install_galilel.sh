...
INSTAL_SNAPSHOTMENU1='$SNAPSHOTMENU1'
...
do
    ...
    echo "";
        ...
        "repair node")
echo -e "${YELLOWMENU}Do you want to repair this broken node with latest bootstrap? [y/n] ${NCMENU}"
read SNAPSHOTMENU
if [[ $INSTAL_SNAPSHOTMENU =~ "Y"|"y" ]] ;
then
systemctl stop ${COIN_NAME}

echo -e "${YELLOWMENU}Do you want to download latest bootstrap? n if you already did [y/n] ${NCMENU}"
read SNAPSHOTMENU1
if [[ $INSTAL_SNAPSHOTMENU1 =~ "Y"|"y" ]] ;
then
echo -e "Download snapshot bootstrap, please wait untill finished"
cd  >/dev/null 2>&1
...
cd  >/dev/null 2>&1
	if [ ! -f bootstrap-latest.tar.gz ]
	then
echo "ERROR: you don't have latest bootstrap file on root"
	else
...
echo "Copy bootstrap file to node folder"
cd  >/dev/null 2>&1
tar xvzf bootstrap-latest.tar.gz -C ${CONFIGFOLDER} >/dev/null 2>&1
sleep 3
systemctl start ${COIN_NAME}
...
echo ""
 ...
 if [ $PILIH = "9" ]
 then
echo -e "${YELLOW}Do you want to delete your bootstrap files? [y/n] ${NC}"
read DELBOOTSTRAP
if [[ $DELBOOTSTRAP =~ "Y"|"y" ]] ;
then
cd >/dev/null 2>&1
...
echo ""
 ...
 if [ $PILIH = "4" ]
then
echo -e "${YELLOW}Do you want to add 4GB swap memory to your server? [y/n] ${NC}"
read ADDSWAP
if [[ $ADDSWAP =~ "Y"|"y" ]] ;
then
...
echo -e "${YELLOW}Write Node that want to DELETE or press enter to exit : ${NC}"
read DEL2
